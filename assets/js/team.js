$(function() {
    ...
    $('#teams-list thead th').each(function() {
        if ($(this).hasClass('s')) {
            var title = $(this).text();
    ...
    /*$('#dsadsadfdg').on('click', function(e) {
        e.preventDefault();

        $.ajax({
            method: 'POST',
            url: lang+"/team/"+$(this).attr('data-id')+"/remove_image",
        })
        .done(function(data) {
            $('.image-wrapper').fadeOut(800);
        });
    });*/
});
